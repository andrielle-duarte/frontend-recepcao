...
const iniciarVisita = async (visitante, atualizarLista) => {
    try {
        ...
        await axios.put(`http://localhost:8000/api/visitantes/${visitante.id}/iniciar`, {
            ...visitante,
...
const alterarMotivo = async (visitanteId, novoMotivo, atualizarLista) => {
    try {
        // Buscar o visitante para atualizar os dados completos (se precisar)
        const res = await axios.get(`http://localhost:8000/api/visitantes/${visitanteId}`);
        const visitante = res.data;

        // Atualizar somente o motivo da visita (os demais dados mantidos)
        await axios.put(`http://localhost:8000/api/visitantes/${visitanteId}`, {
            ...visitante,
...
export default function BuscarResultado() {
    ...
    const buscarVisitantes = () => {
        if (termoBusca) {
            axios
                .get(`http://localhost:8000/api/visitantes/buscar?termo=${termoBusca}`)
                .then((response) => {
