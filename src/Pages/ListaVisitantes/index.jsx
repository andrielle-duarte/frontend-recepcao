...
export default function ListaVisitantes({ atualizar, somenteAtivos = false }) {
  ...
  return (
    <div className="containerLista">
      {somenteAtivos && <h4 className="titulo">Visitantes cadastrados</h4>}
            ...
            {visitantes.map((v) => (
              <tr key={v.id}>
                ...
                {!somenteAtivos && (
                  ...
                  <td>
                  {v.data_entrada
                    ? new Date(v.data_entrada).toLocaleString()
                    : ""}
                </td>
                  )}
                {!somenteAtivos && (
                ...
                <td>
                  {!v.data_saida && (
                    <button className="btnEncerrarVisita" onClick={() => encerrarVisita(v.id)}>
